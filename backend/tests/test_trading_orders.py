...
def swap_supabase_client(monkeypatch):
    ...
    # Monkeypatch global client getter to return our fake
    from backend.supabase_services import __dict__ as services_dict
    services_dict['_client'] = None

    def _get_client():
        return type('C', (), {'client': fake, 'insert': fake.insert, 'update': fake.update, 'delete': fake.delete, 'select': fake.select, 'exists': fake.exists})()

    monkeypatch.setattr('backend.supabase_services.get_client', _get_client)
    # Reset service singletons to force re-init with fake client
